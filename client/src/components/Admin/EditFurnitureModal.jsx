...
const EditFurnitureModal = ({ onClose, onEditSuccess, furnitureItem }) => {
  const [formData, setFormData] = useState({
    name: furnitureItem?.name || '',
    type: furnitureItem?.type || 'chair',
    ...
    color: furnitureItem?.defaultColor || '#ffffff',
    realWorldWidthMeters: furnitureItem?.realWorldWidthMeters || '',
  });
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <h2 className="text-2xl font-bold mb-4 text-gray-900 dark:text-white">Edit Furniture</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">GLB File (Optional - leave blank to keep existing)</label>
            <input
              type="file"
              accept=".glb,.gltf"
